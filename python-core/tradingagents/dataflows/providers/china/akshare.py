...
class AKShareProvider(BaseStockDataProvider):
    ...
    def get_stock_list_sync(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"⚠️ stock_info_a_code_name() 失败，尝试备用接口 stock_zh_a_spot_em(): {e}")
    ...
    async def get_stock_list(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 整个异步操作也限制 3 秒
            try:
                stock_df = await asyncio.wait_for(asyncio.to_thread(fetch_stock_list), timeout=3.0)
            except asyncio.TimeoutError:
                stock_df = None

            if stock_df is None or stock_df.empty:
                logger.warning("⚠️ AKShare 动态获取股票列表失败，尝试使用本地备份文件...")
                try:
                    import json
                    import os
                    # 尝试多个可能的路径
                    possible_paths = [
                        os.path.join(os.getcwd(), "python-core", "config", "stocks_backup.json"),
                        os.path.join(os.getcwd(), "config", "stocks_backup.json"),
                        os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))), "config", "stocks_backup.json")
                    ]
                    ...
                    if backup_path:
                        with open(backup_path, 'r', encoding='utf-8') as f:
                            ...
                            logger.info(f"✅ 成功从本地备份加载 {len(backup_data)} 只股票 (路径: {backup_path})")
                            return backup_data
                    else:
                        logger.error(f"❌ 未找到本地股票备份文件，已尝试路径: {possible_paths}")
                except Exception as e:
                    logger.error(f"❌ 加载本地股票备份失败: {e}")
                return []

            # 转换为标准格式
            stock_list = []
            for _, row in stock_df.iterrows():
                # 兼容不同接口的列名 (code/代码, name/名称)
