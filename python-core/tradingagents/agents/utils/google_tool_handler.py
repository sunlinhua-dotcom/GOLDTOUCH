...
请生成专业、详细的{analyst_type}报告。
        ...
        🌍 语言强制要求 (STRICT LANGUAGE REQUIREMENT):
        1. 你的思考、分析、工具调用参数、最终输出 **必须全过程使用简体中文**。
        2. 🚫 严禁使用英文标题 (如 "Deep Dive", "Analysis", "Conclusion")。
        3. 🚫 严禁直接复制英文原文。如果工具返回英文数据，你 **必须** 先翻译成中文再分析。
        4. 专有名词（如公司名）首次出现可保留英文原名，但必须紧跟中文说明。
        5. **信息综合**: 不要简单罗列数据。必须讲碎片化的数据综合成一段通顺的中文叙述。"""
