...
class ChatGoogleOpenAI(ChatGoogleGenerativeAI):
    ...
    def __init__(self, base_url: Optional[str] = None, **kwargs):
        ...
        if base_url:
            ...
            # 🔄 强制修正：移除版本后缀，因为SDK (google-generativeai) 会自动添加 /v1beta
            # 无论官方域名还是中转域名，行为一致
            if base_url.endswith('/v1beta'):
                api_endpoint = base_url[:-7]
                logger.info(f"🔄 [自动修正] 从 base_url 移除 /v1beta 后缀: {api_endpoint}")
            elif base_url.endswith('/v1'):
                api_endpoint = base_url[:-3]
                logger.info(f"🔄 [自动修正] 从 base_url 移除 /v1 后缀: {api_endpoint}")
            else:
                api_endpoint = base_url
                logger.info(f"🔍 [Check] base_url 未包含常见版本后缀: {api_endpoint}")
