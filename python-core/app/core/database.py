...
from .mongodb_safe import SafeMongoCollection


class SafeMongoDatabase:
    """
    安全的 MongoDB 数据库包装器
    自动包装所有集合，防止 Decimal128 错误
    """
    ...
    def __getattr__(self, name):
        ...
        if hasattr(attr, 'insert_one'):
            return SafeMongoCollection(attr)
        return attr

    async def list_collection_names(self, *args, **kwargs):
        """列出集合名称"""
        return await self._database.list_collection_names(*args, **kwargs)
...
class DatabaseManager:
    ...
    async def init_mongodb(self):
        ...
        try:
            ...
            # 获取数据库实例并包装为安全数据库
            raw_db = self.mongo_client[settings.MONGO_DB]
            self.mongo_db = SafeMongoDatabase(raw_db)
